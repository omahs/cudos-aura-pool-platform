...
String.prototype.replaceAllChar = function replaceAllChar(searchChar: string, replaceChar: string) {
    let lastReplace = -1; let result = ''; const size = this.length;
...
String.prototype.replaceAll = function replaceAll(search: string, replace: string) {
...
String.prototype.replaceAt = function replaceAt(index: number, character: string) {
...
Date.prototype.formatCalendarDate = function formatCalendarDate(words: boolean) {
    if (words === undefined) { words = false; }
...
Date.prototype.formatCalendarDateAndTime = function formatCalendarDateAndTime(words: boolean) {
    if (words === undefined) { words = false; }
...
Date.prototype.formatMonth = function formatMonth(words: boolean) {
    ...
    if (words === false) { return (this.getMonth() + 1).formatSpace(2); }
...
Array.prototype.removeElement = function removeElement(element: any, cmp: (t1: any, t2: any) => boolean) {
...
Array.prototype.pushIfNotExist = function pushIfNotExist(element: any, cmp: (t1: any, t2: any) => boolean) {
    let push = true;
    for (let i = this.length; i-- > 0;) {
        if (cmp !== undefined) {
            if (cmp(this[i], element) === true) {
    ...
    if (push === true) { this.push(element); }
...
Array.prototype.replace = function replace(target: any, cmp: (t1: any, t2: any) => boolean) {
    for (let i = this.length; i-- > 0;) {
...
Map.prototype.equals = function equals(map: Map < any, any >) {
    ...
    let isEquals = true;
    this.forEach((value, key) => {
        if (this.get(key) !== map.get(key)) { isEquals = false; }
    });
...
function initBrowser() {
    ...
    if (userAgent.indexOf('Edge') >= 0) { browser = S.Browser.EDGE; } else if (userAgent.indexOf('Firefox') >= 0) { browser = S.Browser.FIREFOX; } else if (userAgent.indexOf('OPR') >= 0) { browser = S.Browser.OPERA; } else if (userAgent.indexOf('hrome') >= 0) { browser = S.Browser.CHROME; } else if (userAgent.indexOf('afari') >= 0) { browser = S.Browser.SAFARI; } else {
        let versionPosition = S.NOT_EXISTS;

        if (userAgent.indexOf('MSIE') >= 0) { versionPosition = userAgent.indexOf('MSIE') + 4; } else if (userAgent.indexOf('.NET') >= 0) { versionPosition = userAgent.indexOf('rv:') + 3; }

        if (versionPosition !== S.NOT_EXISTS) {
            browser = S.Browser.IE;
            while (userAgent.charAt(versionPosition) === ' ') { ++versionPosition; }

            do {
                browserVersion += userAgent.charAt(versionPosition++);
            } while (userAgent.charAt(versionPosition) >= '0' && userAgent.charAt(versionPosition) <= '9');
        } else { browser = S.Browser.UNKNOWN; }
    }
